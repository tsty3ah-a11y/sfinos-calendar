...
export async function createAppointment({ clientId, title, date, time, durationMinutes, notes }) {
  const { data, error } = await supabase
    .from('appointments')
    .insert({
...
export async function updateAppointment(id, { clientId, title, date, time, durationMinutes, notes }) {
  const { data, error } = await supabase
    .from('appointments')
    .update({
      client_id: clientId || null,
