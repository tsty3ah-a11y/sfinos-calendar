...
export async function updateClientsRoute(clientIds, routeId) {
  const { error } = await supabase
    .from('clients')
    .update({ route_id: routeId })
    .in('id', clientIds);
...
export async function addVisit(clientId, date, status = 'completed') {
  const { data, error } = await supabase
    .from('visits')
    .insert({ client_id: clientId, visit_date: date, status })
    .select()
    .single();
  if (error) throw error;
  return data;
...
// Update a visit's status (e.g. partial -> completed)
export async function updateVisitStatus(clientId, date, status) {
  const { data, error } = await supabase
    .from('visits')
    .update({ status })
